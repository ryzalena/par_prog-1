...
vector<vector<int>> multiplyMatrices(const vector<vector<int>>& A, const vector<vector<int>>& B) {
    ...
    int totalElements = A_rows * B_cols;
    ...
    for (int i = 0; i < A_rows; ++i) {
        for (int j = 0; j < B_cols; ++j) {
            ...
            processedElements++;
            if (processedElements % (totalElements / 20) == 0) { // каждые 5%
                cout << "\rВыполнено: " << (processedElements * 100 / totalElements) << "%" << std::flush;
            }
        }
    }

    cout << "\rВыполнено: 100%" << std::endl;
...
int main() {
    ...
    system("C:\\Users\\456\\PycharmProjects\\matrix\\venv\\Scripts\\python.exe C:\\Users\\456\\source\\repos\\pp_prog_1\\pp_prog_1\\verify_result.py result.txt matrixA.txt matrixB.txt");
